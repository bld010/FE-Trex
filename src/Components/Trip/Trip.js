  ...
  generateLegElements = () => {
    ...
    return this.state.trip.legs.map(leg => {
      return (
        ...
        <Text onPress={() => navigate('Leg', {leg, tripId: this.state.trip.id, userId: this.state.userId})} style={styles.buttonText} key={leg.name}>{leg.startLocation} -> {leg.endLocation}</Text>
      </TouchableOpacity>
      )
    })
  }
